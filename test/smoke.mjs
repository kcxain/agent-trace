...
const requestOnlyTrace = path.join(tmp, "request-only-trace");
const rollouts = path.join(tmp, "rollouts");
const raw = path.join(trace, "raw");
fs.mkdirSync(trace, { recursive: true });
fs.mkdirSync(requestOnlyTrace, { recursive: true });
...
writeJsonl(path.join(requestOnlyTrace, "logs.jsonl"), [event]);
fs.writeFileSync(path.join(requestOnlyTrace, "run.json"), JSON.stringify({ agent: "codex", started_at: "2026-04-26T00:00:00.000Z", command: "codex", args: [] }, null, 2));
execFileSync(process.execPath, [cli, "--generate-md", requestOnlyTrace], { stdio: "pipe" });

const report = fs.readFileSync(path.join(trace, "report-md", "main.md"), "utf8");
const mainReport = report;
const subReport = fs.readFileSync(path.join(trace, "report-md", "subagent-Ada-sub-thre.md"), "utf8");
...
assert(report.includes("## Token Usage Overview"), "report missing token usage overview");
assert(report.includes("| Turn 1 | 5000ms | 800ms | 10 | 4 | 2 |  | 12 |"), "report missing per-turn token usage and timing");
assert(report.includes("| **Total** | 5000ms |  | 10 | 4 | 2 |  | 12 |"), "report missing total token usage and timing");
...
assert(mainReport.includes("[→ Subagent: Ada](subagent-Ada-sub-thre.md)"), "main.md should use report-md-relative subagent links");
assert(mainReport.includes("## Token Usage Overview"), "main.md missing token usage overview");
assert(subReport.includes("| Turn 1 | 700ms | 100ms | 5 | 1 | 1 |  | 6 |"), "subagent md missing token usage and timing overview");
assert(fs.existsSync(path.join(trace, "report-md")), "report-md directory should exist");
assert(fs.existsSync(path.join(trace, "report-md", "main.md")), "report-md/main.md should exist");
assert(!fs.existsSync(path.join(trace, "report.md")), "root report.md should not be generated by default");
assert(fs.existsSync(path.join(requestOnlyTrace, "report-md")), "request-only trace should create report-md directory");
